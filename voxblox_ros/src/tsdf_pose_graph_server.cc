...
TsdfPoseGraphServer::TsdfPoseGraphServer(const ros::NodeHandle& nh,
                       const ros::NodeHandle& nh_private)
    : TsdfPoseGraphServer(nh, nh_private, getTsdfMapConfigFromRosParam(nh_private),
...
TsdfPoseGraphServer::TsdfPoseGraphServer(const ros::NodeHandle& nh,
  ...
  for (int i = 0; i < num_agents_; i++){
      ros::Subscriber tmp_pointcloud_sub = nh_.subscribe(
        "/pointcloud" + std::to_string(i), pointcloud_queue_size_,
                                  &TsdfPoseGraphServer::insertPointcloudTransformWrapper, this);
  ...
  pointcloud_consumer_thread_ = std::thread(&TsdfPoseGraphServer::PointcloudConsumerLoop, this);
  ...
  tsdf_map_sub_ = nh_private_.subscribe("tsdf_map_in", 1,
                                        &TsdfPoseGraphServer::tsdfMapCallback, this);
  ...
  if (use_freespace_pointcloud_) {
    ...
    freespace_pointcloud_sub_ =
        nh_.subscribe("freespace_pointcloud", pointcloud_queue_size_,
                      &TsdfPoseGraphServer::insertFreespacePointcloud, this);
  }
  ...
  generate_mesh_srv_ = nh_private_.advertiseService(
      "generate_mesh", &TsdfPoseGraphServer::generateMeshCallback, this);
  clear_map_srv_ = nh_private_.advertiseService(
      "clear_map", &TsdfPoseGraphServer::clearMapCallback, this);
  save_map_srv_ = nh_private_.advertiseService(
      "save_map", &TsdfPoseGraphServer::saveMapCallback, this);
  load_map_srv_ = nh_private_.advertiseService(
      "load_map", &TsdfPoseGraphServer::loadMapCallback, this);
  publish_pointclouds_srv_ = nh_private_.advertiseService(
      "publish_pointclouds", &TsdfPoseGraphServer::publishPointcloudsCallback, this);
  publish_tsdf_map_srv_ = nh_private_.advertiseService(
      "publish_map", &TsdfPoseGraphServer::publishTsdfMapCallback, this);
  ...
  if (update_mesh_every_n_sec > 0.0) {
    update_mesh_timer_ =
        nh_private_.createTimer(ros::Duration(update_mesh_every_n_sec),
                                &TsdfPoseGraphServer::updateMeshEvent, this);
  }
  ...
  if (publish_map_every_n_sec > 0.0) {
    publish_map_timer_ =
        nh_private_.createTimer(ros::Duration(publish_map_every_n_sec),
                                &TsdfPoseGraphServer::publishMapEvent, this);
  }
}

TsdfPoseGraphServer::~TsdfPoseGraphServer() {
  pointcloud_transform_msgs_received_->Shutdown();
  pointcloud_consumer_thread_.join();
}

void TsdfPoseGraphServer::getServerConfigFromRosParam(
    const ros::NodeHandle& nh_private) {
...
void TsdfPoseGraphServer::processPointCloudMessageAndInsert(
...
bool TsdfPoseGraphServer::getNextPointcloudFromQueue(
...
int pcl_received = 0;
int pcl_counter = 0;
void TsdfPoseGraphServer::insertPointcloudTransformWrapper(
...
void TsdfPoseGraphServer::PointcloudConsumerLoop() {
  comm_msgs::pcl_transformConstPtr pointcloud_transform_msg;
  for (;;) {
    if (pointcloud_transform_msgs_received_->PopBlocking(
           &pointcloud_transform_msg)==false) {
      return;
    }
    sensor_msgs::PointCloud2::Ptr pointcloud_msg_ptr(
       new sensor_msgs::PointCloud2(pointcloud_transform_msg->fusedPointcloud));
    geometry_msgs::Transform geo_transform_msg =
       pointcloud_transform_msg->worldTransform.transform;
...
void TsdfPoseGraphServer::insertTransformPointcloud(
...
// not used in multi agent setup
void TsdfPoseGraphServer::insertPointcloud(
...
void TsdfPoseGraphServer::insertFreespacePointcloud(
    const sensor_msgs::PointCloud2::Ptr& pointcloud_msg_in) {
...
void TsdfPoseGraphServer::integratePointcloud(const Transformation& T_G_C,
...
void TsdfPoseGraphServer::publishAllUpdatedTsdfVoxels() {
...
void TsdfPoseGraphServer::publishTsdfSurfacePoints() {
  // Create a pointcloud with distance = intensity.
...
void TsdfPoseGraphServer::publishTsdfOccupiedNodes() {
  // Create a pointcloud with distance = intensity.
...
void TsdfPoseGraphServer::publishSlices() {
  pcl::PointCloud<pcl::PointXYZI> pointcloud;
...
void TsdfPoseGraphServer::publishMap(bool reset_remote_map) {
...
void TsdfPoseGraphServer::publishPointclouds() {
...
void TsdfPoseGraphServer::updateMesh() {
  if (verbose_) {
...
bool TsdfPoseGraphServer::generateMesh() {
...
bool TsdfPoseGraphServer::saveMap(const std::string& file_path) {
  // Inheriting classes should add saving other layers to this function.
  return io::SaveLayer(tsdf_map_->getTsdfLayer(), file_path);
}

bool TsdfPoseGraphServer::loadMap(const std::string& file_path) {
...
bool TsdfPoseGraphServer::clearMapCallback(std_srvs::Empty::Request& /*request*/,
...
bool TsdfPoseGraphServer::generateMeshCallback(std_srvs::Empty::Request& /*request*/,
                                      std_srvs::Empty::Response&
                                      /*response*/) {  // NOLINT
  return generateMesh();
}

bool TsdfPoseGraphServer::saveMapCallback(voxblox_msgs::FilePath::Request& request,
                                 voxblox_msgs::FilePath::Response&
                                 /*response*/) {  // NOLINT
  return saveMap(request.file_path);
}

bool TsdfPoseGraphServer::loadMapCallback(voxblox_msgs::FilePath::Request& request,
                                 voxblox_msgs::FilePath::Response&
                                 /*response*/) {  // NOLINT
...
bool TsdfPoseGraphServer::publishPointcloudsCallback(
...
bool TsdfPoseGraphServer::publishTsdfMapCallback(std_srvs::Empty::Request& /*request*/,
                                        std_srvs::Empty::Response&
                                        /*response*/) {  // NOLINT
  publishMap();
  return true;
...
void TsdfPoseGraphServer::updateMeshEvent(const ros::TimerEvent& /*event*/) {
  updateMesh();
}

void TsdfPoseGraphServer::publishMapEvent(const ros::TimerEvent& /*event*/) {
  publishMap();
}

void TsdfPoseGraphServer::clear() {
...
void TsdfPoseGraphServer::tsdfMapCallback(const voxblox_msgs::Layer& layer_msg) {
